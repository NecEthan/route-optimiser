...
router.post('/payments', async (req, res) => {
  try {
    ...
    if (status && !validStatuses.includes(status)) {
      return res.status(400).json({
        success: false,
        message: 'Status must be one of: pending, sent, paid, overdue'
      });
    }
...
// PATCH /api/accounting/payments/:id/status - Update payment status
router.patch('/payments/:id/status', async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
    console.log('💰 Updating payment status for ID:', id, 'to status:', status);
    ...
    if (status === 'sent' && req.body.sent_at !== false) {
      updateData.sent_at = new Date().toISOString();
    }
    if (status === 'paid' && req.body.paid_at !== false) {
